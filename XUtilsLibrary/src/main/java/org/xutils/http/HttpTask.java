...
import org.xutils.common.util.TimeUtil;
...
public class HttpTask<ResultType> extends AbsTask<ResultType> implements ProgressHandler {
    ...
    private List<KeyValue> paramsShow;
    ...
    public HttpTask(RequestParams params, Callback.Cancelable cancelHandler,
        ...
        // set params & callback
        if(x.isDebug())
        paramsShow = params.getStringParams();
//        this.paramsShow = paramsShow;
        this.params = params;
    ...
    private void printJsonLog(ResultType result) {
        if(!x.isDebug())return;
        if(result instanceof String) {
            StringBuffer paramTemp=new StringBuffer("");
            for (int i=0;i<paramsShow.size();i++){
                KeyValue keyValue=paramsShow.get(i);
                if (paramsShow.size()==1){
                    paramTemp.append(keyValue.key+"="+keyValue.getValueStr());
                }else{
                    paramTemp.append("&"+keyValue.key+"="+keyValue.getValueStr());
                }
            }
            String param = paramsShow.size() != 0 ? "\n Param ==》" + paramTemp.toString() : "";
            LogUtil.i("结束时间：" + TimeUtil.getCurrentTime() + "=>请求的url: " + this.request.getRequestUri() + param + "\n返回结果==》" + iJsonFormat((String) result, !x.Ext.isFormatLog));
//            LogUtils.i("结束时间：" + TimeUtil.getCurrentTime() + "=>请求的url: " + this.request.getRequestUri() + param + "\n返回结果==》");
//            BeautifulLogUtils.json((String)result);
        }
    }
     ...
     */
    private  String iJsonFormat(String message,boolean isOutputOriginalContent) {
